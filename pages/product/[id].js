...
export default function ProductPage() {
  ...
  const seoDescription = product.description 
    ...
    : `קנה ${product.name} במחיר הטוב ביותר. מוצר איכותי עם ביקורות חיוביות.`;
  ...
  return (
    <>
      <Head>
        <title>{product.name} - Find4U | קנה במחיר הטוב ביותר</title>
        <meta name="description" content={seoDescription} />
        <meta name="keywords" content={seoKeywords} />
        <meta name="author" content="Find4U" />
        <meta name="robots" content="index, follow" />
        
        {/* Open Graph */}
        <meta property="og:title" content={`${product.name} - Find4U`} />
        <meta property="og:description" content={seoDescription} />
        <meta property="og:type" content="product" />
        <meta property="og:url" content={`https://find4u.co.il/product/${productId}`} />
        <meta property="og:image" content={product.image} />
        <meta property="og:site_name" content="Find4U" />
        <meta property="og:locale" content="he_IL" />
        
        {/* Twitter Card */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${product.name} - Find4U`} />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "image": product.image,
              "url": `https://find4u.co.il/product/${productId}`,
              "brand": {
                "@type": "Brand",
                "name": "Find4U"
              },
        ...
        {/* Canonical URL */}
        <link rel="canonical" href={`https://find4u.co.il/product/${productId}`} />
              ...
              "itemListElement": [
                {
                  ...
                  "position": 1,
                  "name": "דף הבית",
                  "item": "https://find4u.co.il"
                },
                {
                  "@type": "ListItem",
                  "position": 2,
                  "name": product.category || "מוצרים",
                  "item": `https://find4u.co.il/category?cat=${product.category}`
                },
                {
                  "@type": "ListItem",
                  "position": 3,
                  "name": product.name,
                  "item": `https://find4u.co.il/product/${productId}`
                }
              ]
            })
          }}
      ...
      <main className={`${styles.mainContainer} ${isDarkMode ? styles.dark : ''}`} dir="rtl">
        ...
        {/* הצגת תיאור המוצר */}
        {product.description && (
          <p className={styles.productDescription} style={{whiteSpace: 'pre-line'}}>{product.description}</p>
        )}
            ...
            <div className={styles.videoContainer}>
              {product.videos.map((videoUrl, index) => {
                ...
                if (videoUrl.includes('tiktok.com') && !videoUrl.includes('/embed/')) {
                  // Try to convert TikTok link to embed format
                  const match = videoUrl.match(/tiktok\.com\/(?:@[^/]+\/video|v)\/(\d+)/);
                  if (match && match[1]) {
                ...
                return (
                  <iframe
                    ...
                    src={embedUrl}
                    scrolling="no"
                    allow={isTikTok ? "autoplay; encrypted-media" : "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"}
                    allowFullScreen
        ...
        {/* מרווח גדול בתחתית כדי שה-SocialBar ו-MobileNav לא יסתירו */}
        <div style={{marginBottom: '90px'}}></div>
        {/* מוצרים דומים */}
        {product && Array.isArray(product.categories) && product.categories.length > 0 && (
          <SimilarProducts currentProductId={product.id} categories={product.categories} />
        )}
      </main>
    </>
  );
...
// רכיב מוצרים דומים
function SimilarProducts({ currentProductId, categories }) {
  const [similar, setSimilar] = useState([]);
  useEffect(() => {
    async function fetchSimilar() {
      const snapshot = await getDocs(collection(db, 'products'));
      const all = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      // מסנן מוצרים מאותה קטגוריה, לא כולל המוצר הנוכחי
      const filtered = all.filter(p =>
        p.id !== currentProductId && Array.isArray(p.categories) && p.categories.some(cat => categories.includes(cat))
      ).slice(0, 6);
      setSimilar(filtered);
    }
    fetchSimilar();
  ...
  return (
    <section style={{marginTop: 40, marginBottom: 40}}>
      <h3 style={{textAlign: 'center', fontWeight: 'bold', fontSize: '1.2rem'}}>מוצרים דומים</h3>
      <div style={{display: 'flex', flexWrap: 'wrap', gap: 20, justifyContent: 'center'}}>
        {similar.map(p => (
          <a key={p.id} href={`/product/${p.id}`} style={{width: 140, textDecoration: 'none', color: '#333', background: '#fafafa', borderRadius: 12, boxShadow: '0 2px 8px #0001', padding: 10, display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
            <img src={p.image} alt={p.name} style={{width: 120, height: 120, objectFit: 'cover', borderRadius: 8, marginBottom: 8}} />
            <div style={{fontWeight: 'bold', fontSize: '1rem', textAlign: 'center'}}>{p.name}</div>
            <div style={{fontSize: '0.9rem', color: '#888'}}>{p.price ? `${Number(p.price).toFixed(2)} ₪` : ''}</div>
          </a>
        ))}
      </div>
    </section>
